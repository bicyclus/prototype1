...
def create_batch():
    ...
    batch = [
        {"startTime": starttime, "endTime": endtime, "groupID": "cwa2", "userID": ID, "sensorData": batch_data,
         "meta": {}}]
    return batch
...
def startGUI():
   ...
   pygame.display.set_caption("Bicyclus Trip")
   pygame.mixer.music.load("Bicycle.ogg")
   bg = (28, 30, 34)
   screen.fill(bg)
   ...
   main = gui.Container(align=-1, valign=-1)

   idlb = gui.Label("User ID:")
   main.add(idlb,120,13)

   idip = gui.Input(size=10)


   idip.connect("activate", idb, idip)
   main.add(idip,185,10)

   b = gui.Button("Pause")
   b.connect(gui.CLICK,pause)
   main.add(b,10,10)

   b = gui.Button("Play")
   b.connect(gui.CLICK, play)
   main.add(b,10,32)
