...
void something(){
  ...
  int N = time_bpm - lastBeatTime;       // monitor the time since the last beat to avoid noise
  ...
  if (N > 250){                                   // avoid high frequency noise£
    if ( (Signal > thresh) && (Pulse == false) && (N > (IBI/5)*3) ){        
      ...
      lastBeatTime = time_bpm;               // keep track of time for next pulse
  ...
  if (N > 2500){                           // if 2.5 seconds go by without a beat
    ...
    P = 512;                               // set P default
    T = 512;                               // set T default
    lastBeatTime = time_bpm;          // bring the lastBeatTime up to date        
